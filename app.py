...
from music.core_engine import musicxml_to_json, json_to_musicxml, finale_musicxml_to_chordcraft_with_layout
...
@app.route("/api/core/musicxml-to-json", methods=["POST"])
def api_core_musicxml_to_json():
    """Core Engine: MusicXML -> JSON canonical"""
    uploaded = request.files.get("file")
    if not uploaded:
        return jsonify({"error": "No MusicXML file uploaded"}), 400
    tmp_dir = EXPORT_DIR / "imports"
    tmp_dir.mkdir(parents=True, exist_ok=True)
    tmp_path = tmp_dir / f"{uuid4().hex}.musicxml"
    try:
        uploaded.save(tmp_path)
        # Detect Finale vs BIAB
        text = tmp_path.read_text(encoding="utf-8", errors="ignore")
        is_finale = "Finale" in text
        if is_finale:
            project = finale_musicxml_to_chordcraft_with_layout(tmp_path)
        else:
            project = musicxml_to_json(tmp_path)
        return jsonify(project)
    except Exception as exc:
        return jsonify({"error": f"Core engine MusicXML->JSON failed: {exc}"}), 400
    finally:
        try:
            tmp_path.unlink(missing_ok=True)
        except Exception:
...
@app.route("/api/core/json-to-musicxml", methods=["POST"])
def api_core_json_to_musicxml():
    """Core Engine: JSON canonical -> MusicXML"""
    data = request.get_json(silent=True) or {}
    try:
        xml_str = json_to_musicxml(data)
        return Response(xml_str, mimetype="application/vnd.recordare.musicxml+xml", headers={"Content-Disposition": 'attachment; filename="export.musicxml"'})
    except Exception as exc:
        return jsonify({"error": f"Core engine JSON->MusicXML failed: {exc}"}), 400


@app.route("/api/project/import-musicxml"
, methods=["POST"])
...
if __name__ == "__main__":
    import os
    app.run(debug=True, port=int(os.environ.get("PORT", 5678)))
